...
# ==========================================
# FUNGSI KONVERSI BASE64 (VISUAL - TIM 2)
# ==========================================
async def fetch_image_as_base64(url: str) -> str:
    """
    Mengunduh gambar dari URL dan mengonversinya ke format Base64 (Data URI).
    Output: String berformat 'data:image/jpeg;base64,...'
    """
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Ubah byte gambar menjadi teks Base64
            base64_encoded = base64.b64encode(response.content).decode('utf-8')
            
            # Tambahkan prefix standar Data URI 
            return f"data:image/jpeg;base64,{base64_encoded}"
    except Exception as e:
        print(f"[ERROR] Gagal mengunduh gambar dari {url}: {e}")
        return None
...
async def perform_visual_validation(keluarga_id: UUID, user_id: UUID, db: Session):
    ...
    # Payload sekarang mengirimkan string Base64 yang panjang, bukan URL
    payload_ke_tim2 = {
        "image_url": base64_luar, 
        "foto_rumah_tampak_dalam": base64_dalam,
        "konteks_rumah": {
    ...
    try:
        ...
        hitung.foto_id_digunakan = foto_tampak_luar.id if foto_tampak_luar else None
        ...
        return {
            ...
            "url_foto_divalidasi": [
                foto_tampak_luar.url_foto if foto_tampak_luar else None,
                foto_tampak_dalam.url_foto if foto_tampak_dalam else None
            ],
...
async def asesmen_visual(keluarga_id: UUID, db: Session = Depends(get_db)):
...
async def asesmen_visual(keluarga_id: UUID, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not hitung:
            hitung = models.Perhitungan(keluarga_id=keluarga_id, user_id=current_user.id)
            db.add(hitung)
...
async def asesmen_komprehensif_semua_tim(
    ...
    try:
        hasil_visual = await asesmen_visual(keluarga_id=id_keluarga, db=db)
    except Exception as e:
