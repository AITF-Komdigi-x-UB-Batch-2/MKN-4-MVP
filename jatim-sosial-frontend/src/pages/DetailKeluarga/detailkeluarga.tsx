...
import { MapPin, FileText, Home, Monitor, Leaf, ShieldCheck, Loader2, ArrowLeft } from 'lucide-react';
...
const DetailKeluarga: React.FC<DetailKeluargaProps> = ({ onLogout }) => {
  ...
  const desilColor = data.desil_nasional <= 3 ? '#ef4444' : data.desil_nasional <= 6 ? '#f97316' : '#22c55e';
      ...
      <div className="detail-keluarga-wrapper">
        ...
        <div className="dk-hero-strip">
          <div className="dk-hero-item">
            ...
            <span className="dk-hero-value">{data.nama_anggota_keluarga || 'Tidak Diketahui'}</span>
          </div>
          <div className="dk-hero-divider" />
          <div className="dk-hero-item">
            <span className="dk-hero-label">NIK / No. Identitas</span>
            <span className="dk-hero-value">—</span>
          </div>
          <div className="dk-hero-divider" />
          <div className="dk-hero-item">
        ...
        <div className="dk-grid">
          ...
          <SectionCard icon={<MapPin size={18} />} title="Informasi Wilayah" color="blue">
            <InfoRow label="Provinsi" value={data.provinsi || data.kode_provinsi} />
          ...
          <SectionCard icon={<FileText size={18} />} title="Identitas & Bantuan" color="purple">
            <InfoRow label="Jumlah Anggota" value={`${data.jumlah_anggota_keluarga || 0} orang`} />
            <InfoRow label="ID Pelanggan PLN" value={data.id_pelanggan_pln || '—'} />
            <InfoRow label="Bantuan Iuran Nasional" value={data.pbi_nas ? 'YA' : 'TIDAK'} />
            <InfoRow label="Bantuan Iuran Pemda" value={data.pbi_pemda ? 'YA' : 'TIDAK'} />
          </SectionCard>
          ...
          <SectionCard icon={<Home size={18} />} title="Kondisi Perumahan" color="green">
            ...
            <InfoRow label="Luas Lantai" value={`${data.luas_lantai || 0} m²`} />
            <InfoRow label="Jenis Lantai Terluas" value={data.jenis_lantai_terluas} />
            <InfoRow label="Jenis Dinding Terluas" value={data.jenis_dinding_terluas} />
            <InfoRow label="Jenis Atap Terluas" value={data.jenis_atap_terluas} />
          </SectionCard>

          {/* Utilitas */}
          <SectionCard icon={<Monitor size={18} />} title="Utilitas & Sanitasi" color="orange">
            <InfoRow label="Sumber Air Minum" value={data.sumber_air_minum_utama} />
            <InfoRow label="Sumber Penerangan" value={data.sumber_penerangan_utama} />
            <InfoRow label="Daya Listrik Terpasang" value={data.daya_terpasang} />
            <InfoRow label="Bahan Bakar Masak" value={data.bahan_bakar_utama_memasak} />
          ...
          <SectionCard icon={<ShieldCheck size={18} />} title="Kepemilikan Aset Bergerak" color="blue">
            <div className="dk-aset-grid">
              <AsetItem label="Tabung Gas" jumlah={data.aset_bergerak_tabung_gas ? 1 : 0} />
              <AsetItem label="Kulkas" jumlah={data.aset_bergerak_lemari_es ? 1 : 0} />
              <AsetItem label="AC" jumlah={data.aset_bergerak_ac ? 1 : 0} />
              <AsetItem label="Water Heater" jumlah={data.aset_bergerak_pemanas_air ? 1 : 0} />
              <AsetItem label="TV" jumlah={data.aset_bergerak_tv_datar ? 1 : 0} />
              <AsetItem label="Emas" jumlah={data.aset_bergerak_emas_perhiasan ? 1 : 0} />
              <AsetItem label="Komputer/Laptop" jumlah={data.aset_bergerak_komputer_laptop_tablet ? 1 : 0} />
              <AsetItem label="Sepeda Motor" jumlah={data.aset_bergerak_sepeda_motor ? 1 : 0} />
              <AsetItem label="Sepeda" jumlah={data.aset_bergerak_sepeda ? 1 : 0} />
              <AsetItem label="Mobil" jumlah={data.aset_bergerak_mobil ? 1 : 0} />
              <AsetItem label="Perahu" jumlah={data.aset_bergerak_perahu ? 1 : 0} />
              <AsetItem label="Kapal Motor" jumlah={data.aset_bergerak_kapal_perahu_motor ? 1 : 0} />
              <AsetItem label="Smartphone" jumlah={data.aset_bergerak_smartphone ? 1 : 0} />
              <AsetItem label="Telepon Rumah" jumlah={data.aset_bergerak_telepon_rumah ? 1 : 0} />
              <AsetItem label="Lahan Lain" jumlah={data.aset_tidak_bergerak_lahan_lainnya ? 1 : 0} />
              <AsetItem label="Rumah Lain" jumlah={data.aset_tidak_bergerak_rumah_lainnya ? 1 : 0} />
            </div>
          </SectionCard>
          ...
          <SectionCard icon={<Leaf size={18} />} title="Kepemilikan Ternak" color="green">
            <div className="dk-aset-grid">
              ...
              <AsetItem label="Kerbau" jumlah={data.jumlah_ternak_kerbau || 0} />
              <AsetItem label="Kuda" jumlah={data.jumlah_ternak_kuda || 0} />
              <AsetItem label="Babi" jumlah={data.jumlah_ternak_babi || 0} />
              <AsetItem label="Kambing/Domba" jumlah={data.jumlah_ternak_kambing_domba || 0} />
            </div>
          </SectionCard>
